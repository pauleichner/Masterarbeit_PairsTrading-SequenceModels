    ...
    for fold, (train_index, test_index) in enumerate(tscv.split(dataFeatures), start=1):
        ...
        class SpreadTransformer(nn.Module):
            def __init__(self, input_size, d_model, nhead, num_layers, dropout=0.1):
                ...
                # self.pos_encoder = PositionalEncodingSCALAR(d_model, max_len=lookback)
        ...
        DFMetrics.to_csv(
            ...
            f"TRANSFORMER_h{horizon}_lb{lookback}"
            f"_nh{NUMLayers}_lr{learningRate}"
        ...
        if fold == n_splits:
            model_path = (
                f"MODELS/TRANSFORMER__BIN_h{horizon}_lb{lookback}"
                f"_nh{NUMLayers}_lr{learningRate}"
                f"_ep{epochs}.pth"
            )
            torch.save({
