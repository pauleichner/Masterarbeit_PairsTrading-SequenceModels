...
def performMultiSpreadTraining(SpreadArray, lookback, horizon, hidden_Size, learningRate, epochs):
    ...
    model       = SpreadLSTM(FeatureLEN, hidden_Size).to(device)
    ...
    df_hist.to_csv(
        f"RESULTS/LSTM_h{horizon}_lb{lookback}_hs{hidden_Size}_lr{learningRate}_ep{epochs}.csv",
        index=False
    )
    torch.save({
    ...
    }, f"MODELS/LSTM_h{horizon}_lb{lookback}_hs{hidden_Size}.pth")
...
if __name__ == "__main__":
    ...
    for horizon in [10, 15]:
        ...
        print("="*40)
        metrics = performMultiSpreadTraining(
            SpreadArray, lookback, horizon,
            hidden_Size, learningRate, epochs
        )
        print(f"Horizon {horizon}: Test RMSE {metrics['RMSE_Test']:.4f}, Test R² {metrics['R2_Test']:.4f}")
